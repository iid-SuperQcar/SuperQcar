...
import math
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
## Gamepad data from remote
gamepad_data = np.array([0,0,0], dtype = np.uint8)
...
try:
	while True:
		...
		# print('Gamepad Input: ', gpad.LB, 0.35, gpad.LLA, gpad.A)
			if X == 1:
				if math.isnan(steering):
					mtr_cmd = control_from_gamepad(LB, 0.35, 0, 0)
					mtr_cmd[1] = 0
				else:
					mtr_cmd = control_from_gamepad(LB, 0.35, 0, 0)
					mtr_cmd[1] = steering
				...
				if math.isnan(steering):
					mtr_cmd = control_from_gamepad(LB, RT, 0, 0)
					mtr_cmd[1] = 0
				else:
					mtr_cmd = control_from_gamepad(LB, RT, 0, 0)
					mtr_cmd[1] = steering
				mtr_cmd[0] = mtr_cmd[0]*np.cos(steering)
			# print('mtr_cmd: ', mtr_cmd)
